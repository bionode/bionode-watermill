...
reducer.resolveOutput = (task) => (dispatch) => new Promise((resolve, reject) => {
  ...
  const resolver = require('../resolver2.js')
  const matchToFs = require('../matchers/match-to-fs.js')

  return resolver(task.output, (item) => matchToFs(item, null))
  .then((resolvedOutput) => {
    dispatch({
      type: RESOLVE_OUTPUT,
      uid: task.uid,
  ...
  // const matchToFs = (pattern) => new Promise((resolve, reject) => globby(pattern).then(resolve).catch(reject))
  //
  // matchToFs(task.output.file)
  // .then((val) => {
  //   const resolvedOutput = val[0]
  //
  //   dispatch({
  //     type: RESOLVE_OUTPUT,
  //     uid: task.uid,
...
reducer.runValidators = (task) => (dispatch) => new Promise((resolve, reject) => {
  dispatch({
    type: TASK_LOG,
    uid: task.uid,
  ...
  const validations = Promise.all([
    validators.matchHash(resolvedOutput)
  ])
  ...
  .catch((err) => {
    ...
    dispatch({
      type: TASK_LOG,
      uid: task.uid,
      content: tab(1) + 'A validator failed: ' + err
    })
