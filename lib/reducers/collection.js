...
function addOutputHandler (state, action) {
  ...
  const { uid, output, input, params, context, name } = action
  ...
  if (name) {
    ...
    graph.addNewVertex(uid, {taskName: name, input: input, output: output, params: params}) //addNewVertex(key, [value])
  }
...
reducer.addOutput = (uid, taskState) => ({
  ...
  input: taskState.input,     // ideally we should pass resolvedInput
