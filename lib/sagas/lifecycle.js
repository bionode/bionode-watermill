...
fs.writeFile('manifest.txt', 'Folder name to task name...\n\n')
...
function* lifecycle (action) {
  ...
  // redefines folder for with task name
  //console.log(`miniUid: ${miniUid} ${originalTask.name}`)
  let manifest_log = `folder name: ${miniUid} -> task name: ${originalTask.name}\n`
  fs.appendFile('manifest.txt', manifest_log, function (err) {
    if (err) {
