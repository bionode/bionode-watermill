...
function* lifecycle (action) {
  ...
  function* operationSaga (operationCreator) {
    ...
    try {
      ...
      let linkedResolvedInput = null
      if ((yield select(selectTask(uid))).resolvedInput) {
        linkedResolvedInput = yield call(setupTaskdir, yield(select(selectTask(uid))), nestedLogger(1))
      }
      ops = yield call(
        createOperation,
