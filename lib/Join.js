  ...
  return function(resolvedOutput) {
    ...
    const doTaskAndTryNext = (i, tasks, resolvedOutput, trajectory) => {
      ...
      const finish = once((task) => {
        ...
        // TODO this better
        const parmsStr = JSON.stringify(task.stateWormhole.params)
        if (task.stateWormhole.trajectory === undefined) {
          task.stateWormhole.trajectory = []
        }
        task.stateWormhole.trajectory.push(parmsStr)

        store[parmsStr] = { outputs: {} }
        Object.assign(store[parmsStr].outputs, { [task.stateWormhole.uid]:output })

        // if (parmsStr !== '{}') {
        //   store[parmsStr] = { outputs: { [task.stateWormhole.uid]:output } }
        // }
        // else {
        //   console.log(task.stateWormhole.trajectory)
        //   Object.assign(store[parmsStr].outputs, { [task.stateWormhole.uid]:output })
        // }
