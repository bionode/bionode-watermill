    ...
    const accumulator = (currentCtx, task, i, length) => new Promise((resolve, reject) => {
      ...
      if (task.info.type === 'fork') {
        ...
        task.info.tasks.forEach((forkee, j) => {
          ...
          // makes an hash for downStreamTasks
          ...
          console.log('down: ', downStreamUids)
          ...
          if (firstFork === true) {
            ...
            const newUpStreamTasks = taskCreationDispatcher(dispatch, upStreamTasks, task.info.uid, forkee.info.uid, downStreamUids)
            console.log(newUpStreamTasks)
            ...
            if (forkee.info.props) {
              ...
              if (outermostTasks.info !== upStreamTasks.info) {
                console.log("teste2")
                const newUpStreamTasks = taskCreationDispatcher(dispatch, upStreamTasks, task.info.uid, forkee.info.uid, downStreamUids)
                const newOutermostTasks = taskCreationDispatcher(dispatch, outermostTasks, task.info.uid, forkee.info.uid, downStreamUids)
                ...
                console.log(task.info, forkee.info)
                const newOutermostTasks = taskCreationDispatcher(dispatch, outermostTasks, task.info.uid, forkee.info.uid, downStreamUids)
              ...
              // for join and junction imediatly after a fork in order to
...
// the fork's task
const taskCreationDispatcher = (dispatch, taskInstance, forkUid, forkeeUid, downStreamUid) => {
  console.log("taskInstance: ", taskInstance, forkUid, forkeeUid, downStreamUid)
  const taskNewUid = taskInstance.map(
    t => createTask(dispatch)(
              t.info.props, t.info.operationCreator, hash(t.info.uid + forkUid + forkeeUid + downStreamUid)
            )
  )
