...
const join = (dispatch, logger) => {
  return function (...tasks) {
    ...
    const accumulator = (currentCtx, task, i, length) => new Promise((resolve, reject) => {
      ...
      if (task.info.type === 'fork' && firstFork === true) {
        console.log("testeste: ", firstFork)
        console.log('fork encountered inside join! ', task.info)
        firstFork = false
        const joinages = []
...
const taskCreationDispatcher = (dispatch, taskInstance, forkUid, forkeeUid) => {
  ...
  const taskNewUid = taskInstance.map(
    t => createTask(dispatch)(
      t.info.props, t.info.operationCreator, hash(t.info.uid + forkUid + forkeeUid)
    )
  )
  console.log(taskNewUid)
  return taskNewUid
}
