      ...
      if (task.info) {
        ...
        if (task.info.type === 'fork') {
          ...
          task.info.tasks.forEach((forkee, j) => {
            ...
            console.log("teste",i, restTasks)


            console.log("fork_bla")
            console.log(tasks)
            console.log(forkee)
            console.log(forkee.info)
            ...
            if (forkee.info.tasks) {
              forkee.info.tasks.forEach((subTask, k) => {
                ...
                if (subTask.info.type === 'fork') {
                  ...
                  console.log(subTask.info.tasks) // tasks within the inner fork
                  console.log(forkee.info.tasks) // tasks in outer fork
                  ...
                  subTask.info.tasks.forEach((subForkee, j2) => {
                    ...
                    const newUpTasks = upStreamTasks.map(
                      t => createTask(dispatch)(
                        t.info.props, t.info.operationCreator, hash(t.info.uid + j2)
                      )
                    )

                    const newDownTasks = downStreamTasks.map(
                      t => createTask(dispatch)(
                        t.info.props, t.info.operationCreator, hash(t.info.uid + j2)
                      )
                    )
                    ...
                    const lineage = newDownTasks.concat([subForkee]).concat(newUpTasks).concat(newRestTasks)
                  ...
                  console.log("yes_array but no fork")
