          ...
          task.info.tasks.forEach((forkee, j) => {
            ...
            if (forkee.info.tasks) {
              forkee.info.tasks.forEach((subTask, k) => {
                ...
                } else if (subTask.info.type === 'junction') {
                  // executed when there is a junction inside a fork
                  // NOTE: join would disrupt this behavior and add a new
                  // vertex to the graph without appending the outer array
                  // of junctions newResTasks and join is intended to create
                  // a sequential portion of code rather than handling
                  // several graph flows.
                  console.log('junction inside fork!')
                  const lineage = downStreamTasks.concat([subTask]).concat(upStreamTasks).concat(newRestTasks)
                  lineageGetter(dispatch, lineage, joinages)
                } else if (subTask.info.type === 'join') {
                  // if join is inside fork we do not want to duplicate any
                  // tasks, then we use restTasks as well as downStramTasks
                  // and upStreamTasks without the duplicated event tasks 'new*'
                  // NOTE: for junction we really want to duplicate every
                  // task within the junction so we used newRestTasks but
                  // here with join we do not want to duplicate it.
                  console.log('join inside fork!')
                  const lineage = downStreamTasks.concat([subTask]).concat(upStreamTasks).concat(restTasks)
                  lineageGetter(dispatch, lineage, joinages)
                }
              })
            } else {
