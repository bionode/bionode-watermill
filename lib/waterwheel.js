...
const taskYAML = (obj) => {
  if (typeof(obj.input) === 'string') {
    obj.input = new File(obj.input)
  }

  if (obj.output instanceof Array && typeof(obj.output[0]) === 'string') {
    obj.output.map(str => new File(str))
  }

  const t = task({
    input: obj.input,
    output: obj.output,
  }, ({ input }) => shell(`${eval(obj.task)}`))
