 ...
 */
const task = (store, actions) => (props, operationCreator) => (passedOutput, trajectory) => {
  ...
  .then((uid) => {
    if (getTask(uid).validated) {
      ...
      .then(uid => actions.createAction(getTask(uid), operationCreator))
      .then(uid => actions.setDuplex(getTask(uid), stream))
      .then(([uid, streamType]) => actions.catchStream(getTask(uid), stream, streamType))
