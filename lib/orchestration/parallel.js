...
function parallel (...tasks) {
  return (trajectory = []) => {
    ...
    const all = Promise.all(tasks.map((task, i) => new Promise((resolve, reject) => {
      ...
      // if (task.toString().indexOf('making a parallel') >= 0) {
      //   console.log('PARALLEL on PARALLEL')
      // }
      ...
      // liveTask.on('parallel.finish', () => console.log('received parallel.finish'))
    })))
    ...
    emitter.async = new Promise((resolve, reject) => {
      emitter.on(PARALLEL_FINISH, (results) => {
        // console.log('got a parallel.finish: ', results)
        resolve({ trajectory: results })
      })
    })
