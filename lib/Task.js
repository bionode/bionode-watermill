...
const task = (store, actions) => (props, actionCreator) => (passedOutput) => {
  ...
  .then((uid) => getTask(uid).resolvedOutput ? actions.runValidators(getTask(uid)) : Promise.resolve(uid))
  // .then(uid => actions.runValidators(getTask(uid)))
  .then((uid) => {
