 ...
 * @returns 'foo'
 */
const valueResolver = (key, obj, passed) => {
  const val = obj[key]

  console.log(tab(2) + 'resolved ' + chalk.magenta(val) + (passed ? ' from passed in ' : ' from ') + chalk.cyan('value'))
  return val
}
...
const task = (props, actionCreator) => (passedOutput) => {
  ...
  if (passedOutput) {
    ...
    resolvedInput = traverser(passedOutput, (key, obj) => {
      ...
      switch(key) {
        case 'value':
          obj = valueResolver(key, obj, true)
          break
        case 'file':
          obj = fileResolver(key, obj, 'passed', input)
          break
        case 'resolved':
          break
        default:
    ...
    resolvedInput = traverser(input, (key, obj) => {
      ...
      switch(key) {
        case 'value':
          obj = valueResolver(key, obj, false)
          break
        case 'file':
          obj = fileResolver(key, obj, 'start')    
      }
